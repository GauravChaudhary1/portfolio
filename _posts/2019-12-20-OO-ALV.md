---
title: "OO-ALV with basis functionalities"
description: "Object Oriented Approach to display the records of table in ALV with Basis functionalities like Sorting, Grouping, Sum, Header, Footer, Events, etc."
categories: [abap]
tags: [ooalv]
---

<p>
Reporting in ABAP is considered as major requirement as it caters the need of the business enduser. There are a lot of functions that can be used while displaying the data in ALV. For the beginners it seems quite difficult to know them all in place.
</p>

<p> Below program executes such functionalities of ALV:</p> <br>
...
FIELD-SYMBOLS : <lfs_vbak> TYPE t_vbak.
...
** Set Label and Text Link **
  ...
  lr_picture->set_picture_id( 'ZGAUR' ).
  lr_grid->set_element( row = 3
                        column = 1
...
** Move lr_grid to lr_content **
  ...
  PERFORM end_of_page CHANGING gr_content.
...
*----------------------------------------------------------------------*
FORM set_color.
  LOOP AT lt_vbak ASSIGNING <lfs_vbak>.
    IF <lfs_vbak>-netwr GT 100.
      REFRESH : lt_color.

      CLEAR : ls_color.
      MOVE 'VBELN' TO ls_color-fname.
      MOVE 5       TO ls_color-color-col.
      ...
      MOVE lt_color TO <lfs_vbak>-t_color.
    ENDIF.
  ENDLOOP.
ENDFORM.                    "set_color

FORM end_of_page CHANGING lr_content TYPE REF TO cl_salv_form_element.
  DATA: lr_grid TYPE REF TO cl_salv_form_layout_grid.

  CREATE OBJECT lr_grid.

  lr_grid->create_label( row = 1
                         column = 1
...
<h3> Output generated by this program:</h3>

<img src="{{site.url}}{{site.baseurl}}/images/abap1-20191220/output.png">
